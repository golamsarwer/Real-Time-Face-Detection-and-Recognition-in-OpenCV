...
def draw_boundary(img,classifier,scaleFactor, minNeighbors, color, text, clf):
    ...
    for (x,y,w,h) in features:
        ...
        if id == 1:
            ...
            coords = [x,y,w,h]
        if id == 2:
            cv2.putText(img, "Miraz", (x,y-4),cv2.FONT_HERSHEY_SIMPLEX,0.8, color,1,cv2.LINE_AA)
            coords = [x,y,w,h]
        if id == 3:
            cv2.putText(img, "Ammu", (x,y-4),cv2.FONT_HERSHEY_SIMPLEX,0.8, color,1,cv2.LINE_AA)
            coords = [x,y,w,h]
        if id == 4:
            cv2.putText(img, "Riaz", (x,y-4),cv2.FONT_HERSHEY_SIMPLEX,0.8, color,1,cv2.LINE_AA)
            coords = [x,y,w,h]
